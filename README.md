...
| Throughput | **v0.23.4 d03 regression, measured and understood, not fixed:** the production-faithful maxdom3 canary is **51.67% slower** than v0.23.3 (2.9831176877 vs 1.9668543059 s/root-step); 2-domain configurations are unaffected (0.960–1.018×). Hardware-measured occupancy/latency-bound, not bandwidth-bound. The accepted full one-hour nine-nest replay used 5,744.407 s warm model wall, but that correctness stress workload is not a matched benchmark. Historical v0.20 figures remain in [docs/PERFORMANCE.md](docs/PERFORMANCE.md). |
